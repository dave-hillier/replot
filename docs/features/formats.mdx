import {formatMonth, formatWeekday, Replot, TextX} from "@dave-hillier/replot/react";
...
```js
formatNumber("en-US")(Math.PI) // "3.142"
```
...
```js
formatIsoDate(new Date("2020-01-01T00:00:00.000Z")) // "2020-01-01"
```
...
<Replot x={{tickFormat: formatWeekday()}}>
  <TextX data={d3.range(7)} />
</Replot>

```js
formatWeekday("es-MX", "long")(0) // "domingo"
```
...
<Replot x={{tickFormat: formatMonth(), ticks: 12}}>
  <TextX data={d3.range(12)} />
</Replot>

```js
formatMonth("es-MX", "long")(0) // "enero"
```
