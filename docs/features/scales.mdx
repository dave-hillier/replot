import {BarY, BinX, Cell, CellX, Dot, Frame, LineY, RectY, Replot, RuleX, RuleY, TickX, WindowY, identity, scale} from "@dave-hillier/replot/react";
import * as d3 from "d3";
import {useState} from "react";
import gistemp from "../data/gistemp.ts";
import aapl from "../data/aapl.ts";
import goog from "../data/goog.ts";
import sftemp from "../data/sf-temperatures.ts";

export const color = scale({color: {scheme: "Tableau10", domain: ["apple", "orange", "pear"]}});

export const anomaly = gistemp.map((d) => d.Anomaly);

export function PaddingAlignDemo() {
    ...
    <div>
      <div>
        <label className="label-input">
          <span>Padding:</span>
          <input type="range" value={padding} onChange={(e) => setPadding(Number(e.target.value))} min="0" max="1" step="0.01" />
          <span style={{fontVariantNumeric: "tabular-nums"}}>{padding.toFixed(2)}</span>
        </label>
        <label className="label-input">
          <span>Align:</span>
          <input type="range" value={align} onChange={(e) => setAlign(Number(e.target.value))} min="0" max="1" step="0.01" />
          <span style={{fontVariantNumeric: "tabular-nums"}}>{align.toFixed(2)}</span>
        </label>
      </div>
      <Replot grid marginTop={0.5} x={{padding, align, round: false}}>
        <Frame strokeOpacity={0.3} />
        <TickX data={"ABCDEFGHIJ"} x={identity} stroke="currentColor" />
      </Replot>
      <Replot grid marginTop={0.5} x={{padding, align, round: false}}>
        <Frame strokeOpacity={0.3} />
        <Cell data={"ABCDEFGHIJ"} x={identity} stroke="currentColor" />
      </Replot>
    </div>
  );
}

export function SchemeQDemo() {
  const [schemeq, setSchemeq] = useState("turbo");
  return (
    <div>
      <div>
        <label className="label-input">
          Color scheme:
          <select value={schemeq} onChange={(e) => setSchemeq(e.target.value)}>
      ...
      <Replot axis={null} padding={0} color={{scheme: schemeq}}>
        <Cell data={d3.range(40)} x={identity} fill={identity} inset={-0.5} />
      </Replot>
    </div>
  );
}

export function InterpolateQDemo() {
  const [interpolateq, setInterpolateq] = useState("rgb");
  return (
    <div>
      <div>
        <label className="label-input">
          Color interpolate:
          <select value={interpolateq} onChange={(e) => setInterpolateq(e.target.value)}>
      ...
      <Replot axis={null} padding={0} color={{
          type: "linear",
...
export function SchemeDDemo() {
  const [schemed, setSchemed] = useState("rdbu");
  return (
    <div>
      <div>
        <label className="label-input">
          Color scheme:
          <select value={schemed} onChange={(e) => setSchemed(e.target.value)}>
      ...
      <Replot axis={null} padding={0} color={{type: "linear", scheme: schemed}}>
        <Cell data={d3.range(40)} x={identity} fill={identity} inset={-0.5} />
      </Replot>
    </div>
  );
...
export function SchemeODemo() {
  const [schemeo, setSchemeo] = useState("Observable10");
  return (
    <div>
      <div>
        <label className="label-input">
          Color scheme:
          <select value={schemeo} onChange={(e) => setSchemeo(e.target.value)}>
      ...
      <Replot color={{type: "ordinal", scheme: schemeo}}>
        <Cell data={"ABCDEFGHIJ"} x={identity} fill={identity} />
      </Replot>
    </div>
  );
}
  ...
  return (
    <div>
      <div>
        <label className="label-input">
          Radius:
          <input type="range" value={radius} onChange={(e) => setRadius(Number(e.target.value))} min="1" max="20" step="0.1" />
          <span style={{fontVariantNumeric: "tabular-nums"}}>{radius.toFixed(1)}</span>
        </label>
      </div>
      <Replot r={{range: [0, radius]}}>
        <Dot data={d3.range(1, 11)} x={identity} r={identity} fill="currentColor" />
      </Replot>
    </div>
  );
}
...
export function IntervalDemo() {
  const [intervaled, setIntervaled] = useState(true);
  return (
    <div>
      <div>
        <label className="label-input">
          Use interval:
          <input type="checkbox" checked={intervaled} onChange={(e) => setIntervaled(e.target.checked)} />
        </label>
      </div>
      <Replot marginBottom={80} x={{
          tickRotate: -90,
...
In Replot, the [mark](./marks.md) binds channels to scales; for example, the line’s **x** channel is bound to the *x* scale. The channel name and the scale name are often the same, but not always; for example, an area’s **y1** and **y2** channels are both bound to the *y* scale. (You can opt-out of a scale for a particular channel using [scale overrides](./marks.md#mark-options) if needed.)
...
<Replot>
  <RuleX data={gistemp} x="Date" stroke="Anomaly" />
</Replot>
...
Within a given [plot](./plots.md), marks share scales. For example, if a plot has two line marks, such as the lines below visualizing the daily closing price of <span style={{borderBottom: "solid 2px var(--vp-c-red)"}}>Google</span> and <span style={{borderBottom: "solid 2px var(--vp-c-blue)"}}>Apple</span> stock, both share the same *x* and *y* scales for a consistent encoding.

<Replot>
  <RuleY data={[0]} />
  <LineY data={goog} x="Date" y="Close" stroke="red" />
...
Replot has many different scales; we categorize them by their _input_ (**domain**) and _output_ (**range**).
...
<Replot x={{domain: [0, 100], reverse: true, grid: true}} />

If the domain is dates, Replot will default to a UTC scale. This is a linear scale with ticks based on the Gregorian calendar.

{/* Replot doesn’t parse dates; convert your strings to [Date](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Date) instances with [d3.utcParse](https://d3js.org/d3-time-format#utcParse) or [d3.autoType](https://d3js.org/d3-dsv#autoType). */}

<Replot x={{domain: [new Date("2021-01-01"), new Date("2022-01-01")], grid: true}} />
...
<Replot x={{type: "utc", domain: [1609459200000, 1640995200000], grid: true}} />

If the scale **type** is *time*, the ticks will be in local time — as with the dates below — rather than UTC.

<Replot x={{type: "time", domain: [new Date(2021, 0, 1), new Date(2022, 0, 1)], grid: true}} />
...
<Replot x={{type: "log", domain: [1e0, 1e5], grid: true}} />
...
<Replot x={{type: "log", domain: [1e0, 1e5], tickFormat: ",", grid: true}} />
...
<Replot x={{type: "log", base: 2, domain: [1e0, 1e4], ticks: 20, grid: true}} />
...
<Replot x={{type: "symlog", domain: [-10, 10], grid: true}} />
...
<Replot x={{type: "sqrt", domain: [0, 100], grid: true}} />

<Replot x={{type: "pow", exponent: 1 / 3, domain: [0, 100], grid: true}} />
...
<Replot x={{type: "point", domain: "ABCDEFGHIJ", grid: true}} />
...
<Replot x={{type: "band", domain: "ABCDEFGHIJ"}}>
  <Cell data={"ABCDEFGHIJ"} x={identity} stroke="currentColor" strokeOpacity={0.1} />
</Replot>
...
Below we again show observed global surface temperatures. The reversed *BuRd* color scheme is used since <span style={{borderBottom: "solid 2px steelblue"}}>blue</span> and <span style={{borderBottom: "solid 2px #b2182b"}}>red</span> are semantically associated with cold and hot, respectively.
...
Replot also provides color schemes for discrete data. Use the *categorical* type for categorical (nominal) unordered data, and the *ordinal* type for ordered data.
...
Replot strives to be concise: rather than you laboriously specifying everything, Replot can guess by inspecting the data so you don’t have to set the **type**, **domain**, and **range** (and for color, **scheme**) of scales explicitly. But for Replot’s guesses to be accurate, your data must match Replot’s expectations. Here they are.

A scale’s **type** is most often inferred from associated marks’ channel values: strings and booleans imply an *ordinal* scale; dates imply a *utc* scale; anything else is *linear*. Replot assumes that your data is consistently typed, so inference is based solely on the first non-null, non-undefined value. We recommend typed CSV (passing `{typed: true}` to Observable’s FileAttachment csv method) or explicitly coercing types when loading data (*e.g.*, d3.autoType).
...
<Replot y={{grid: true, label: "Temperature (°C)", transform: (f) => (f - 32) * (5 / 9)}}>
  <RuleY data={[32]} />
  <WindowY k={7}>
...
<Replot y={{percent: true}} color={{scheme: "BuRd"}}>
  <BinX y="proportion" fill="x">
    <RectY data={gistemp} x="Anomaly" fill="Anomaly" />
  </BinX>
  <RuleY data={[0]} />
</Replot>
...
As an added bonus, the **fontVariant** and **type** options are no longer needed because Replot now understands that the *x* scale, despite being *ordinal*, represents daily observations.
...
UTC is recommended over local time as charts in UTC time are guaranteed to appear consistently to all viewers whereas charts in local time will depend on the viewer’s time zone. Due to limitations in JavaScript’s Date class, Replot does not yet support an explicit time zone other than UTC.
...
The normal scale types — *linear*, *sqrt*, *pow*, *log*, *symlog*, and *ordinal* — can be used to encode color. In addition, Replot supports special scale types for color:
...
<Replot width={322} height={25 * 27} margin={0} marginRight={70} padding={0} x={{axis: null}} y={{axis: "right", tickSize: 0}} color={{type: "identity"}}>
  <Cell data={[
...
The default color scheme, *turbo*, was chosen primarily to ensure high-contrast visibility. Color schemes such as *blues* make low-value marks difficult to see against a white background, for better or for worse. To use a subset of a continuous color scheme (or any single-argument *interpolate* function), set the *scale*.range property to the corresponding subset of [0, 1]; for example, to use the first half of the *rainbow* color scheme, use a range of [0, 0.5]. By default, the full range [0, 1] is used. If you wish to encode a quantitative value without hue, consider using *opacity* rather than *color* (e.g., use Replot.dot’s *strokeOpacity* instead of *stroke*).

The following diverging scale schemes are supported:

<Replot width={322} height={25 * 11} margin={0} marginRight={70} padding={0} x={{axis: null}} y={{axis: "right", tickSize: 0}} color={{type: "identity"}}>
  <Cell data={[
...
<Replot width={322} height={25 * 2} margin={0} marginRight={70} padding={0} x={{axis: null}} y={{axis: "right", tickSize: 0}} color={{type: "identity"}}>
  <Cell data={[
...
<Replot width={322} height={25 * 10} margin={0} marginRight={70} padding={0} x={{axis: null}} y={{axis: "right", tickSize: 0}} color={{type: "identity"}}>
  <Cell data={[
      ["Accent", d3.schemeAccent],
      ["Category10", d3.schemeCategory10],
      ["Dark2", d3.schemeDark2],
      ["Observable10", scale({color: {type: "categorical"}}).range],
...
Replot implicitly generates an [axis mark](../marks/axis.md) for position scales if one is not explicitly declared. (For more control, declare the axis mark explicitly.) The following [axis mark options](../marks/axis.md#axis-options) are also available as scale options, applying to the implicit axis:
...
```jsx
<BarY data={alphabet} x="letter" y="frequency" sort={{x: "y", reduce: "max", order: "ascending"}} />
```
...
```jsx
<BarY data={alphabet} x="letter" y="frequency" sort={{x: "y", order: "descending"}} />
```
...
```jsx
<BarY data={alphabet} x="letter" y="frequency" sort={{x: "y", reverse: true}} />
```
...
```jsx
<BarY data={alphabet} x="letter" y="frequency" sort={{x: "y", limit: 5}} />
```
...
```jsx
<BarY data={alphabet} x="letter" y="frequency" sort={{x: {value: "y", order: "descending"}}} />
```
...
Note: when the value of the sort option is a string or a function, it is interpreted as a mark [sort transform](../transforms/sort.md). To use both sort options and a mark sort transform, use [Replot.sort](../transforms/sort.md#sort).

## scale(*options*) <VersionBadge version="0.4.0" /> {/* {#scale} */}
You can also create a standalone scale with the **scale**(*options*) function, available as the `scale` export of `@dave-hillier/replot/react`. The *options* object must define at least one scale; see [Scale options](#scale-options) for how to define a scale. For example, here is a categorical color scale with the *Tableau10* color scheme and a domain of fruits:

```js
const color = scale({color: {scheme: "Tableau10", domain: ["apple", "orange", "pear"]}});
```

The [scale](#scale) function returns a scale object. These objects represent the actual (or “materialized”) scale options used by Replot, including the domain, range, interpolate function, *etc.* The scale’s label, if any, is also returned; however, note that other axis properties are not currently exposed. Point and band scales also expose their materialized bandwidth and step.
...
To apply a standalone scale object to a plot, pass it to the `<Replot>` component as the corresponding scale prop, such as **color**:

<Replot color={color}>
  <CellX data={["apple", "apple", "orange", "pear", "orange"]} />
</Replot>
