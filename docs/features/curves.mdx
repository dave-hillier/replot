import {DotY, LineY, Replot} from "@dave-hillier/replot/react";
import * as d3 from "d3";
import {useState} from "react";

export const numbers = d3.range(20).map(d3.randomLcg(42));

export function CurveDemo() {
  const [curve, setCurve] = useState("catmull-rom");
  return (
    <div>
      <p>
      ...
      <Replot>
        <LineY data={numbers} curve={curve} />
        <DotY data={numbers} x={(d, i) => i} />
      </Replot>
    </div>
  );
}
