import {AreaY, BarX, BarY, Dot, LineY, Replot, RuleY} from "@dave-hillier/replot/react";
...
export function AreaToggle() {
  ...
  return (
    <div>
      ...
      <Replot>
        <RuleY data={[0]} />
        {area ? <AreaY data={aapl} x="Date" y="Close" fillOpacity={0.2} /> : null}
...
# Marks {/* {#Marks} */}
If you aren't yet up and running with Replot, please read our [getting started guide](../getting-started.md) first. Tinkering with the code below will give a better sense of how Replot works.
Replot doesn't have chart types; instead, you construct charts by layering **marks**.


Every mark is available as a [React component](../getting-started.md#plot-in-react) (e.g., `<Dot data={data} x="x" y="y" />`). The examples on this page use the React components.
## Marks are geometric shapes

Replot provides a variety of mark types. Think of marks as the "visual vocabulary" — the painter's palette, but of shapes instead of colors — that you pull from when composing a chart. Each mark type produces a certain type of geometric shape.

For example, the [dot mark](../marks/dot.md) draws stroked circles (by default).

<Replot>
  <Dot data={gistemp} x="Date" y="Anomaly" />
</Replot>
...
<Replot>
  <RuleY data={[0]} />
  <AreaY data={aapl} x="Date" y="Close" fillOpacity={0.2} />
  <LineY data={aapl} x="Date" y="Close" />
</Replot>
...
<Replot>
  <LineY data={goog} x="Date" y="Close" />
  <LineY data={aapl} x="Date" y="Close" />
</Replot>

Marks may also be a function which returns an [SVG element](https://developer.mozilla.org/en-US/docs/Web/SVG/Element), if you wish to insert arbitrary content.

<Replot>
  <BarY data={alphabet} x="letter" y="frequency" fill="steelblue" />
  <RuleY data={[0]} />
</Replot>
...
Marks are (typically) not positioned in literal pixels, or colored in literal colors, as in a conventional graphics system. Instead you provide abstract values such as time and temperature — marks are drawn "in data space" — and [scales](./scales.md) encode these into visual values such as position and color. And best of all, Replot automatically creates [axes](../marks/axis.md) and [legends](./legends.md) to document the scales' encodings.
...
<Replot>
  <LineY data={useBlsMetroUnemployment()} x="date" y="unemployment" z="division" />
</Replot>

Replot favors [tidy data](http://vita.had.co.nz/papers/tidy-data.html) structured as an array of objects, where each object represents an observation (a row), and each object property represents an observed value; all objects in the array should have the same property names (the columns).
...
<Replot>
  <LineY data={linedata} x="hour" y="value" stroke="sensor" />
</Replot>
...
<Replot>
  <LineY data={linedata} x={(d) => d.hour} y={(d) => d.value} stroke={(d) => d.sensor} />
</Replot>

For greater efficiency, Replot also supports columnar data: you can use an [Apache Arrow](https://arrow.apache.org/docs/js/) table as data instead of an array of objects. <VersionBadge version="0.6.16" pr="2115" /> You can even pass parallel arrays of values, or Apache Arrow vectors, to each channel.
...
Because nominal values often need some arbitrary order for display purposes — often alphabetical — Replot uses the term *ordinal* to refer to both ordinal and nominal data.
...
<Replot>
  <LineY data={["please", "don't", "do", "this"]} />
</Replot>


While Replot aspires to give good defaults and helpful warnings, Replot won't prevent you from creating a meaningless chart. *Only you* can prevent bogus charts!
In particular, beware the simple "bar"! A bar mark is used for a bar chart, but a rect mark is needed for a histogram. Replot has four different mark types for drawing rectangles:
...
Replot encourages you to think about data types as you visualize because data types often imply semantics. For example, do you notice anything strange about the bar chart below?
...
```jsx
<Replot x={{tickFormat: ""}}>
  <RectY data={timeseries} x="year" y="population" interval={1} />
</Replot>
```
...
```jsx
<Replot x={{tickFormat: "", interval: 1}}>
  <BarY data={timeseries} x="year" y="population" />
</Replot>
```
...
<Replot>
  <Dot data={gistemp} x="Date" y="Anomaly" fill="red" />
</Replot>

As the name suggests, options are generally optional; Replot tries to provide good defaults for whatever you don't specify. Replot even has [shorthand](./shorthand.md) for various common forms of data. Below, we extract an array of numbers from the `gistemp` dataset, and use the line mark shorthand to set *x* = index and *y* = identity.

<Replot>
  <LineY data={gistemp.map((d) => d.Anomaly)} />
</Replot>

Some marks even provide default [transforms](./transforms.md), say for [stacking](../transforms/stack.md)!


Because Replot strives to be concise, there are many default behaviors, some of which can be subtle. If Replot isn't doing what you expect, try disabling the defaults by specifying options explicitly.
...
Not all mark options can be expressed as channels. For example, **stroke** can be a channel but **strokeDasharray** cannot. This is mostly a pragmatic limitation — it would be harder to implement Replot if every option were expressible as a channel — but it also serves to guide you towards options that are intended for encoding data.
...
Some options can be either a channel or a constant depending on the provided value. For example, if you set the **fill** option to *purple*, Replot interprets it as a literal color.

<Replot y={{label: null, tickFormat: ""}}>
  <BarX data={timeseries} x="population" y="year" fill="purple" />
</Replot>

Whereas if the **fill** option is a string but *not* a valid CSS color, Replot assumes you mean the corresponding column of the data and interprets it as a channel.

<Replot y={{label: null, tickFormat: ""}}>
  <BarX data={timeseries} x="population" y="year" fill="year" />
</Replot>

If the **fill** option is a function, it is interpreted as a channel.

<Replot y={{label: null, tickFormat: ""}}>
  <BarX data={timeseries} x="population" y="year" fill={(d) => d.year} />
</Replot>
...
<Replot y={{label: null, tickFormat: ""}}>
  <BarX data={timeseries} x="population" y="year" fill={(d) => d.year & 1 ? "red" : "currentColor"} />
</Replot>

But rather than supplying literal values, it is more semantic to provide abstract values and use scales. In addition to centralizing the encoding definition (if used by multiple marks), it allows Replot to generate a legend.

<Replot y={{label: null, tickFormat: ""}} color={{legend: true}}>
  <BarX data={timeseries} x="population" y="year" fill={(d) => d.year & 1 ? "odd" : "even"} />
</Replot>
...
```
```jsx
<Dot data={index} x={units} y={fruits} />
```
...
Missing and invalid data are handled specifically for each mark type and channel. In most cases, if the provided channel value for a given datum is null, undefined, or (strictly) NaN, the mark will implicitly filter the datum and not generate a corresponding output. In some cases, such as the radius (*r*) of a dot, the channel value must additionally be positive. Replot.line and Replot.area will stop the path before any invalid point and start again at the next valid point, thus creating interruptions rather than interpolating between valid points. Titles will only be added if they are non-empty.
