import {Dot, Replot, RuleY} from "@dave-hillier/replot/react";
...
The **color** legend supports quantitative, ordinal, and categorical scales. For quantitative scales, a gradient ramp is displayed; for ordinal and categorical scales, swatches are shown. The **opacity** legend shows an opacity ramp for quantitative opacity scales.
...
The **style** legend option allows custom styles to override Replot’s defaults; it has the same behavior as in Replot’s top-level [plot options](./plots.md). The **className** option is suffixed with *-ramp* or *-swatches*, reflecting the **legend** type.
