import {Dot, LinearRegressionX, LinearRegressionY, Replot} from "@dave-hillier/replot/react";
    ...
    <div>
      <p>
        <label className="label-input">
          Number of points (m):
          <input type="range" value={m} onChange={(e) => setM(Number(e.target.value))} min="0" max={cars.length} step="1" />
          <span style={{fontVariantNumeric: "tabular-nums"}}>{m.toLocaleString("en-US")}</span>
        </label>
      </p>
      <Replot>
        <Dot data={cars} x="weight (lb)" y="power (hp)" fill="currentColor" fillOpacity={0.2} />
        <Dot data={cars.slice(0, m)} x="weight (lb)" y="power (hp)" />
        <LinearRegressionY data={cars.slice(0, m)} x="weight (lb)" y="power (hp)" stroke="red" />
      </Replot>
    </div>
  );
}
...
<Replot>
  <Dot data={cars} x="weight (lb)" y="power (hp)" />
  <LinearRegressionY data={cars} x="weight (lb)" y="power (hp)" stroke="red" />
</Replot>
...
<Replot>
  <Dot data={cars} x="weight (lb)" y="power (hp)" strokeOpacity={0.5} r={2} />
  <LinearRegressionY data={cars} x="weight (lb)" y="power (hp)" stroke="steelblue" />
  <LinearRegressionX data={cars} x="weight (lb)" y="power (hp)" stroke="orange" />
</Replot>
...
```jsx
<LinearRegressionX data={mtcars} y="wt" x="hp" />
```
