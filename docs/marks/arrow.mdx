import {Arrow, Dot, Replot, Text} from "@dave-hillier/replot/react";
...
export function samegroup({source, target}) {
  const s = miserables.groups.get(source);
  const t = miserables.groups.get(target);
  return s === t ? s : null;
}
...
<Replot grid inset={10} x={{
  ...
  <Arrow data={metros} x1="POP_1980" y1="R90_10_1980" x2="POP_2015" y2="R90_10_2015" bend stroke={(d) => d.R90_10_2015 - d.R90_10_1980} />
  <Text data={metros} x="POP_2015" y="R90_10_2015" filter="highlight" text="nyt_display" fill="currentColor" stroke="var(--vp-c-bg)" dy={-6} />
</Replot>
...
<Replot inset={60} aspectRatio={1} axis={null}>
  <Dot data={markov.nodes} r={40} />
  <Arrow data={markov.edges} x1={([[x1]]) => x1} y1={([[, y1]]) => y1} x2={([, [x2]]) => x2} y2={([, [, y2]]) => y2} bend strokeWidth={([,, value]) => value} strokeLinejoin="miter" headLength={24} inset={48} />
  <Text data={markov.nodes} text={["A", "B", "C"]} dy={12} />
  <Text data={markov.edges} x={([[x1, y1], [x2, y2]]) => (x1 + x2) / 2 + (y1 - y2) * 0.15} y={([[x1, y1], [x2, y2]]) => (y1 + y2) / 2 - (x1 - x2) * 0.15} text={([,, value]) => value} />
</Replot>
...
<Replot height={1080} marginLeft={100} axis={null} x={{domain: [0, 1]}} color={{domain: d3.range(10), unknown: "#ccc"}}>
  <Dot data={miserables.nodes} x={0} y="id" fill="group" sort={{y: "fill"}} />
  <Text data={miserables.nodes} x={0} y="id" text="id" textAnchor="end" dx={-6} fill="group" />
  <Arrow data={miserables.links} x={0} y1="source" y2="target" sweep="-y" bend={90} headLength={0} stroke={samegroup} sort={samegroup} reverse />
</Replot>
...
```jsx
<Arrow data={inequality} x1="POP_1980" y1="R90_10_1980" x2="POP_2015" y2="R90_10_2015" bend />
```
